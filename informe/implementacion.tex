...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/topology_n1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/topology_n2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/topology_n3.png}
...
\textbf{Ubicación del Firewall (Parametrizable):}

El firewall permite configurar en qué switches se aplican las reglas mediante el campo \texttt{switch} en cada regla del archivo JSON. Por defecto, las reglas se aplican en el switch 1 (primer switch de la cadena). Esta decisión de diseño tiene las siguientes implicaciones:

\begin{itemize}
    \item \textbf{Ventaja}: Centraliza el control de seguridad en un único punto, simplificando la gestión y monitoreo.
    \item \textbf{Limitación}: El tráfico que no pasa por S1 (ej: comunicación directa entre h3 y h4 en topologías N$>$1) no es filtrado si las reglas solo están configuradas para S1.
    ...
    \item \textbf{Flexibilidad}: Se pueden configurar reglas específicas para cada switch según las necesidades de segmentación de la red.
\end{itemize}

La implementación permite especificar el campo \texttt{"switch": N} en cada regla del archivo JSON. Al conectarse un switch, el controlador filtra las reglas correspondientes a su DPID y solo instala aquellas que le corresponden. Si no se especifica el campo, la regla se aplica al switch 1 por defecto.
\\
...
\begin{itemize}
    ...
    \item Validación del número de switch (entero $\geq$ 1)
    ...
    \item Asignación automática de switch por defecto (1) si no se especifica
...
\begin{itemize}
    ...
    \item \texttt{test\_suite.py}: Script interactivo que automatiza la ejecución de las pruebas, validando conectividad y reglas del firewall
\end{itemize}
